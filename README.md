...
这是一个前后端分离的支教团队网站，包含公共首页、团队风采、课件共享、互动留言、AI 小助手和用户管理。
...
5. 最终固定顺序是 `00 → 10 → 11 → 12 → 13 → 14 → 15 → 20 → 30 → 31`。仍须在数据库备份副本中用真实 MySQL 完整重跑；本轮没有数据库凭据，不能把静态检查写成实测通过。
...
4. 学生账号：访问 `/regs` 注册学生，登录后发布留言、回复留言；确认不能进入管理员页面。
5. 权限：未登录直接访问受保护的管理地址应跳到登录页；学生访问管理员地址应被拒绝。
6. 课件：用团队账号上传一份无敏感信息的样例课件，检查预览和下载。
7. AI：只有配置有效 DeepSeek Key 后再测试；Key 只能放环境变量，不能写进源码。
...
- 前端已完成干净安装及完整/生产依赖审计，当前均为 0 个已知漏洞；后续升级依赖仍需重新执行完整回归。
