...
| 5 | `13_public_image_quota.sql` | 公共图片永久数量/容量原子账本、稳定资源编号 | 已提供 |
| 6 | `20_message_board.sql` | 留言与回复查询索引 | 已提供 |
| 7 | `30_material_center.sql` | 课件中心字段、索引和上传会话表 | 已提供 |
| 8 | `40_volunteer_application.sql` | 志愿者报名表及处理中手机号唯一约束 | 已提供 |
...
2. 先执行根目录 `ganlu.sql`。
3. 完整顺序固定为 `00 → 10 → 11 → 12 → 13 → 20 → 30 → 40`；其中部分变更已经进入基线，脚本会跳过已存在的索引或表。
