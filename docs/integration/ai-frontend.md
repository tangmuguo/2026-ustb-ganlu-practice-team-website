...
- 最近 20 条消息的多轮会话；请求上下文单条不超过 2000 字、总计不超过 32000 字；
...
前端以 `AI_MAX_MESSAGE_LENGTH = 2000` 和 `AI_MAX_CONTEXT_LENGTH = 32000` 作为接口合同常量。助手完整回答保留在页面和当前会话中；再次请求时，所有消息统一经 `buildContextMessages` 处理：单条超长助手回答仅在请求副本中截取前 2000 字，累计超限则从最旧的完整 `user/assistant` 轮次开始删除，且始终保留本轮用户问题。发送、失败重试和会话恢复后的再次发送共用这条路径。
...
- 页面与会话存储保留助手完整回答；发往后端的副本才按单条 2000 字和总计 32000 字限制裁剪。
...
| AI-FE-14 | Mock 首轮返回 2001 字助手回答，再发送第二个问题 | 页面完整显示 2001 字；第二次请求中每条消息均不超过 2000 字，后端不返回 400 | 通过；页面保留 2001 字，第二次请求最大消息长度为 2000 并成功返回 |
| AI-FE-15 | 构造累计内容超过 32000 字后继续提问 | 请求总长度不超过 32000；从 user 开始、历史问答成对，最后一条为本轮 user | 通过；仅淘汰最旧完整问答轮次，请求总长度 30000，角色严格交替 |

验证环境：Windows 11、Node.js 24.18.0、npm 11.16.0、Microsoft Edge 无头模式。2026-08-01 按 PR #9 审核意见完成账号隔离、请求中清空、第 11 轮上下文、超长助手回答和 32000 字总量限制专项回归。本地 Mock 不消耗真实 AI 额度。由于项目基线中的 `@videojs-player/vue@1.0.0` 与 `video.js@8` 存在 peer dependency 冲突，npm 11 安装时使用了 `npm ci --legacy-peer-deps --cache .npm`，未修改 `package.json` 或锁文件。
